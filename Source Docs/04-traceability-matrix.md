...
| **DR_104** ‡ | DR | 구역 경계 이벤트 발행 | **Zone segmentation correctness** | **Not yet agreed — highest priority** |
| VIS_103 | VIS | 농작물 상태 판단 | `detections[].state` | Contract proposed |
| VIS_104 | VIS | 분석 결과 생성 | Analysis JSON per image | Contract proposed |
| VIS_104.4 | VIS | 판단불가 처리 | Coverage denominator, `재촬영_필요` flag | Contract proposed |
...
|---|---|---|---|---|
| WEB_101.2 | WEB | Markdown report to render | C3.2, six fixed sections | Proposed |
| WEB_101.3 | WEB | 구역별 생육 현황 data | C3.3 `metadata.json` | Proposed |
| WEB_104.1 | WEB | 농작물 대시보드 data | C3.3 `zones[]` | Proposed |
