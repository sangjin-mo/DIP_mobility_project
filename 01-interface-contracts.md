...
> [!FLAG] `ai_report/models.py` is hand-written, not `datamodel-codegen`-generated, for one reason: `Detection.confidence` being null only when `state == 판단불가` (rule C2.3) and the `patrol_id` format both need validation JSON Schema's `enum`/`pattern`/`required` can't fully express as one cross-field rule. The model is still schema-backed — every field matches its schema field-for-field, `contracts/validate.py` checks fixtures against the schemas, and a manual round-trip (fake_rover/fake_vis output validated through the real schemas) is re-checked whenever a model or schema changes. Drift is caught by that process, not by codegen.
...
|---|---|---|---|---|
| **C1** | DR → AI | UDP + HTTP | `AGREED` | DR team — will implement to match `contracts/schemas/c1-telemetry.schema.json` / `c1-event.schema.json` |
| **C2** | VIS → AI | filesystem + JSON | `AGREED` | VIS team — will implement to match `contracts/schemas/c2-analysis.schema.json` |
| **C3** | AI → WEB | filesystem | `AGREED` | WEB team — will implement to match `contracts/schemas/c3-metadata.schema.json` |

> [!FLAG] **Sign-off recorded 2026-08-13**
> DR, VIS, and WEB agreed to implement against the AI team's schemas as
> written, rather than a separate negotiated draft — so `AGREED` here means
> "AI-authored and adopted," not "jointly redrafted." Two follow-ups this
> creates, tracked in `03-build-plan.md`:
> 1. `c1-telemetry.schema.json` predates this sign-off and was already
>    matched field-for-field by `ai_report/models.py`; `c1-event` and
>    `c2-analysis` were authored in the same pass, from this document's
>    prose. `c3-metadata.schema.json` is new and has no Python model yet —
>    A2's `pipeline/aggregate.py` is what will produce real
>    `metadata.json` output and should be checked against it then.
> 2. A schema change is still expensive after A4 even under this
>    arrangement — DR/VIS/WEB matching us doesn't mean the shape is free
>    to change once they've built against it. Treat `AGREED` here with the
>    same weight a jointly-negotiated freeze would carry.
