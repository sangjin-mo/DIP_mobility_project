# PiRacer drive: web + Vision GPIO stop + YDLiDAR X2
...
| Vision Pi GPIO17 is HIGH | Dashboard state becomes `STOPPED`; throttle is zero. | Press web START again. |
...
a HIGH pulse on BCM GPIO17 and one authenticated `STOP` to the drive Pi. GPIO
can be disabled with `--no-gpio` only when the wire is not connected.
...
The drive Pi reports final drive state on BCM GPIO27: `0` means moving and `1`
means stopped. Monitor it on the Vision Pi with:

```bash
python vision_drive_status.py --gpio-pin 27
```

Connect GPIO17 (Vision Pi output) to GPIO17 (Drive Pi input), GPIO27 (Drive Pi
output) to a Vision Pi input, and connect the grounds. Use common ground and
3.3 V GPIO logic only; do not connect 5 V to a GPIO pin. These are BCM numbers,
