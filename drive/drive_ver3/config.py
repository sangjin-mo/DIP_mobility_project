...
# YDLiDAR X2 USB obstacle safety. The gate is placed immediately before the
# motor actuator path. If enabled, a missing/failed LiDAR blocks driving.
LIDAR_SAFETY_ENABLED = False
LIDAR_PORT = "/dev/ttyUSB0"
# The X2's minimum measurable distance is about 0.10~0.12 m.  Stop with a
# margin so an object never has to be measured at its unreliable limit.
LIDAR_STOP_DISTANCE_M = 0.15
# Inspect only the 30-degree sector directly ahead of the sensor.  Steering
# remains entirely under the trained line-following model.
LIDAR_FORWARD_CENTER_DEG = 0.0
LIDAR_FORWARD_HALF_ANGLE_DEG = 15.0
LIDAR_CLEAR_SCANS_REQUIRED = 3
LIDAR_FAIL_SAFE_STOP = True

# Optional Vision Pi GPIO stop input/output. Set enabled after wiring and
# verifying the pins with the wheels raised.
VISION_GPIO_STOP_ENABLED = False
VISION_GPIO_STOP_PIN = 17
VISION_GPIO_STATUS_ENABLED = False
