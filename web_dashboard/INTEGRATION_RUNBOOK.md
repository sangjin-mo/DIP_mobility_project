...
순찰 촬영은 차량 제어에 묶여 있다. 대시보드의 START가 웹캠 Pi에
`POST :8002/capture/start`를, STOP이 `POST :8002/capture/stop`을 보낸다 —
버튼을 누른 그 순간에 바로 반영된다. 카메라가 안 켜지면 START 응답의
`camera` 필드에 실패 이유가 담겨 돌아온다(주행은 그대로 진행된다).

별도의 "촬영" 버튼(단발 촬영)은 여전히 차량 상태와 무관하게 동작하는
수동 오버라이드다.
...
`촬영 주기` 입력란에는 0.2~10초를 지정할 수 있다. `주기 적용`을 누르면
