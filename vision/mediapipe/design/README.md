...
> 작성일: 2026-08-22
> 상태: **보류** (2026-08-23) — 2호기 실기에서 mediapipe 실행 불가 블로커(§4-5) 해결 전까지 중단. 코드는 남겨두되 추가 작업은 보류
...
이 파일들은 전부 **2호기(비전 Pi) → 1호기 직접** 패턴이고 PC를 거치지 않는다. 처음엔 "정지는 안전 요구사항이니 PC가 꺼져 있어도 되어야 한다"는 이유로 이 직결형을 채택했었으나, 아래처럼 **재정정**했다.

| 경로 | 방식 | 채택 여부 |
|---|---|---|
| 직결형 (2호기 → 1호기 직접) | `vehicle_control_client.py`로 1호기에 직접 인증된 STOP/START | 2026-08-22 한때 채택 → **이후 철회**. 안전상으로는 더 낫지만, 실제로 구현·운영해보니 채널이 하나 더 늘어나는 복잡도(토큰 별도 관리, `vehicle_control_client.py` 유지보수)가 부담됐고, 사용자가 "표지판 정지도 그냥 웹 정지 버튼과 같은 경로로 하면 된다"고 판단해 되돌림 |
| **경유형 (PC `web_dashboard` 거침, 최종 채택)** | 2호기 → `dashboard_client.stop()` → PC `web_dashboard`의 `POST /api/control/stop` → 1호기. **대시보드의 "■ 정지" 버튼과 완전히 동일한 경로** | **2026-08-22 최종 확정.** `vehicle_control_client.py` 삭제, `GestureController`가 `dashboard_client` 하나만 사용. **트레이드오프를 명시적으로 감수**: PC가 꺼지거나 PC↔1호기 연결이 끊기면 표지판/주먹을 인식해도 정지가 전달되지 않음. 다만 이 경우 하트비트(§3-1-4)도 같은 PC 경로로 끊기므로 1호기가 워치독(1.5초)으로 어차피 스스로 정지함 — PC 완전 다운 시나리오는 결과적으로 안전. 진짜 남는 리스크는 "PC는 살아있는데 PC↔1호기만 끊기는" 좁은 경우 |
...
- [x] **결정: PC `/api/control/status` 폴링 채택** (2026-08-22) — `state_api.py`의 `/vehicle-state`는 아무도 호출해주지 않는 문제가 있었음(`stop_sign/design/README.md` §8). `detector.py`에 `poll_driving_state()` 스레드를 추가해 `dashboard_client.get_status()`(기존 코드 재사용, 신규 클라이언트 불필요)로 1초 주기 폴링, `state == "RUNNING"` → `vehicle_state.set_driving(True)`. 조회 실패 시 안전 쪽으로 `driving=False` 처리. 기존 push용 `/vehicle-state` 엔드포인트는 수동 테스트 편의를 위해 그대로 유지(§3-1-보충)
...
├── detector.py                   (수정) 표지판 인식 루프에 제스처 인식 통합, GPIO 제거, driving 상태 폴링 추가
├── dashboard_client.py           (수정) PC web_dashboard 경유로 정지·재출발까지 통합 — get_status/stop/set_speed/send_heartbeat
├── config.py                     (수정) GPIO_OUT_PIN 제거, DRIVING_STATE_POLL_INTERVAL_S 추가
└── requirements.txt              (수정) gpiozero 제거
```
> `vehicle_control_client.py`(1호기 직접 STOP/START)는 2026-08-22에 만들었다가, 정지도 PC 경유로 통합하기로 재결정하면서 삭제함 (위 표 참고).
...
### 4-9. 크롭/회전 버그 원인 확정 및 수정 — 실제 손동작을 따라가기 시작함

로그만 보며 값을 조정하는 방식의 한계를 인정하고, 디텍터 박스·keypoint를 원본 프레임에 그려서 시각적으로 확인하는 방식으로 전환함.

**선행 발견 — 카메라가 천장을 보고 있었음**: 디버그 스냅샷을 찍어보니 2호기의 C920 카메라가 사람이 아니라 **천장(조명·환풍구)을 향해 거치돼 있었음**. 그동안 손이 프레임 가장자리에 걸치듯 잘려서 잡혔던 건 이 물리적 각도 때문이었음.

**버그 1 — 크롭 중심 좌표 계산 오류**: `cv2.getRotationMatrix2D(center, ...)`는 반환 행렬이 `center`를 그 자리에 고정한 채 회전·스케일만 적용한다(이 시점엔 `(cx,cy)`가 여전히 `(cx,cy)`에 있음). 그런데 그 점을 출력 이미지 중앙으로 옮기는 이동량 계산에서 `out_size/2 - cx * scale_factor`처럼 **`cx`에 `scale_factor`를 잘못 곱해서** 이동량이 크게 부족했고, 크롭이 항상 원래 위치보다 중앙 쪽으로 쏠려 손 대신 배경만 잡히고 있었음. **수정**: `out_size/2 - cx`(스케일 곱하지 않음)로 교체.
- 수정 전/후 비교: presence가 항상 0.00~0.02이던 것이 수정 후 **0.79~1.00**으로 즉시 뛰어오름 — 이 버그가 핵심 원인이었음이 확인됨

**버그 2 — 회전 방향 부호**: `cv2.getRotationMatrix2D`에 넘기던 각도에 불필요한 음수 부호(`-math.degrees(rotation)`)가 붙어 있어서 손이 옆으로 90도가량 돌아간 채 크롭됐음. **수정**: 음수 부호 제거.

**결과 1 — 자유 동작 테스트**: 사용자가 즉석에서 손동작을 바꿔가며 10초 테스트함. presence 0.9대 이상 유지, 취한 동작(주먹→브이→손바닥)을 따라 분류 결과가 바뀜을 확인:
```
[ 2029ms] presence=0.98 -> Closed_Fist=0.77, None=0.23
[ 4412ms] presence=1.00 -> Victory=0.59, None=0.40
[ 6391ms] presence=0.99 -> Open_Palm=0.68, None=0.32
```

**결과 2 — 따봉/역따봉 집중 테스트, 아직 불안정**: 따봉만 집중적으로 보여준 10초 테스트에서는 presence는 여전히 높았지만(0.92~1.00), 분류는 `Closed_Fist`(초반)와 `None`(후반) 사이를 오갔고 `Thumb_Up`은 최고 0.30(7240ms, 그마저도 1위는 아님)까지만 올라감 — **주먹만큼 안정적이지 않음**. 카메라가 여전히 위(천장)를 보고 있어서 따봉/역따봉처럼 엄지 방향이 핵심인 제스처를 아래에서 올려다보는 매우 비정상적인 각도로 촬영하게 되는 것이 원인으로 추정됨(주먹은 어느 각도에서 봐도 실루엣이 비슷해 상대적으로 덜 민감했을 가능성). 카메라 각도를 정면으로 바꾼 뒤 재검증 필요.

### 4-10. 다음 단계

1. **카메라 거치 각도를 사람/손 쪽 정면으로 재조정** — 지금 천장을 보고 있는 게 따봉/역따봉 인식 불안정의 유력한 원인. 최우선
2. 정면 각도로 재조정 후 3개 제스처(주먹/따봉/역따봉) 각각 디바운스(연속 N프레임) 적용 시 실사용 가능한 안정성이 나오는지 재확인
3. `shift_y`/`scale` 추가 미세조정 여지 있음(지금은 대략값 scale=2.0, shift_y=0)
4. `gesture_controller.py`(이미 단위 테스트로 검증된 상위 로직)와 이 v2 파이프라인을 실제로 연결
5. mediapipe 소스 재빌드나 다른 하드웨어에서 인식을 돌리는 방안은 이 자체 파이프라인으로 대체 가능해져 **불필요해짐**

### 4-11. gesture_controller 통합 테스트 — 1차 실패, 2차 성공

`raw_pipeline_v2.run_pipeline()`을 `gesture_controller.on_frame()`이 기대하는 `(label, score)|None` 형태로 변환하는 어댑터를 만들어, 실제 카메라로 `vehicle_client`/`dashboard_client`(Mock)까지 이어붙여 봄(`extracted/integration_test.py`).

**1차 시도(15초): 명령이 한 번도 안 나감.** 손은 계속 잡혔지만(presence 0.7~0.9대) 분류가 거의 전 구간에서 `None`(confidence 0.9대)으로 나와, 목표 3개 제스처가 임계값을 넘긴 적이 없었음.

**2차 시도(20초): 성공 — STOP 2회, 속도변경 1회, 전부 정확히 대응됨.**
```
[ 3370ms] Closed_Fist(0.95), 3프레임 연속 → [control] STOP 전송 완료 (사유={'FIST'})
[14063ms] Closed_Fist(0.91), 3프레임 연속 → [control] STOP 전송 완료 (사유={'FIST'}) (두 번째 주먹, 재트리거도 정상)
[16888ms] Thumb_Up 3프레임 연속(0.69→0.60→0.49) → [gesture] 속도 변경 0.25 -> 0.3 m/s
```
최종 집계: `send_stop` 2회, `set_speed` 1회(인자 `0.3` 확인), `send_start` 0회(설계대로 — 주먹 풀어도 자동 재출발 안 함, §4-2).

`Thumb_Down`은 7607ms·9422ms에 단발로만 잡혀서 3프레임 연속을 못 채워 걸러짐 — 이는 버그가 아니라 **디바운스가 의도대로 노이즈를 걸러낸 것**(연속 유지 시간이 짧았을 뿐).

**결론**: 1차 실패는 raw 분류 자체의 근본적 문제가 아니라 **그 순간 제스처를 충분히 오래·명확하게 유지하지 못한 것**(2차 시도에서 똑같은 코드로 성공)이 유력한 원인으로 확인됨. 카메라가 천장을 향한 각도 문제는 여전히 남아있어(따봉/역따봉이 주먹보다 덜 안정적인 이유로 유력) 정면 각도로 재조정하면 안정성이 더 올라갈 것으로 기대되지만, 지금 각도로도 정지·가속 명령이 실제로 성공한 사례를 확보함.

- [ ] `gesture_config.py`의 `DRIVE_PI_CONTROL_URL`/`DRIVE_PI_CONTROL_TOKEN`/`DASHBOARD_URL`을 실제 값으로 교체
- [ ] 2호기 실기에서 표지판 인식 + 제스처 인식(자체 파이프라인) 동시 구동 시 CPU 부하 실측
- [ ] 디바운스 N/M, confidence 임계값, 쿨타임 2초 값 실측 후 조정 — 특히 오탐지(§4-4) 재현 여부를 실제 사이드카메라 각도로 다시 확인 필요
- [ ] 실제 1호기·PC 장비와 end-to-end 통합 테스트 (지금까지는 Mock)
- [ ] 카메라 거치 각도를 사람을 향하도록 재조정 후 따봉/역따봉 안정성 재검증
- [ ] 역따봉(`Thumb_Down`)이 충분히 오래 유지됐을 때도 디바운스를 통과하는지 재확인 (이번엔 유지 시간이 짧아 미확인)

## 5. 결론

**성공 — 이 2호기 하드웨어에서 자체 구현 파이프라인이 정지·가속 명령까지 실제로 정상 동작함을 실측으로 확인함.**

- ✅ 공식 mediapipe는 이 Pi(Cortex-A72, AES 미지원)에서 크래시하고 우회법도 없음(§4-5) — **확정**
- ✅ 그 아래 TFLite+XNNPACK 엔진은 이 CPU에서 정상 동작함 — **확정**
- ✅ 손 검출(팜 디텍터)을 anchor 디코딩+NMS까지 구현해서 높은 신뢰도(0.85~0.98)로 안정적으로 손을 찾아냄 — **확정**
- ✅ 크롭/회전 버그 2건(중심 좌표 계산 오류, 회전 부호 오류)을 찾아 수정, presence가 0.01→0.9대로 즉시 개선됨 — **확정**
- ✅ 주먹→STOP, 따봉→속도변경까지 `gesture_controller` 포함 end-to-end로 실제 동작 확인(§4-11 2차 시도) — **확정**
- ⚠️ 역따봉은 이번엔 유지 시간이 짧아 디바운스를 못 넘김 — 버그인지 단순히 시연이 짧았는지 재확인 필요 — **미확정**
- ⚠️ 카메라가 여전히 천장을 보는 각도라 안정성에 영향 줄 수 있음 — 정면 재조정 후 재검증 권장 — **개선 여지**
- [ ] 지금까지 전부 Mock(`vehicle_client`/`dashboard_client`) 기준 — 실제 1호기·PC 연동 테스트는 아직 안 함

**요약**: MediaPipe 공식 API는 이 하드웨어에서 못 쓰지만, 서브 모델을 직접 이어 붙인 자체 파이프라인으로 손동작 인식뿐 아니라 정지/가속 명령 디스패치까지 실제로 동작한다는 걸 오늘 세션에서 end-to-end로 증명했다. 카메라 각도 개선과 실제 하드웨어(1호기/PC) 연동이 남은 주요 과제.
