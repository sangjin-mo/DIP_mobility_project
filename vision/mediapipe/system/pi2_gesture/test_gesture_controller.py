...
# vehicle_client/dashboard_client는 Mock으로 대체 (design/README.md 검증 계획 §1).
#
...
def make_controller():
    ...
    vehicle_client.send_stop.return_value = {"accepted": True, "state": "STOPPED", "target_speed_mps": 0.0}
    vehicle_client.send_start.return_value = {"accepted": True, "state": "RUNNING", "target_speed_mps": 0.25}
    ...
    controller = GestureController(vehicle_client, dashboard_client)
    return controller, vehicle_client, dashboard_client
...
class FistStopTests(unittest.TestCase):
    def test_requires_debounce_before_stopping(self):
        controller, vehicle_client, _ = make_controller()
        feed(controller, FIST, cfg.GESTURE_DEBOUNCE_N - 1)
        vehicle_client.send_stop.assert_not_called()
    ...
    def test_stop_fires_once_debounce_satisfied(self):
        ...
        vehicle_client.send_stop.assert_called_once()
    ...
    def test_releasing_fist_does_not_auto_resume(self):
        ...
        vehicle_client.send_start.assert_not_called()
    ...
    def test_fist_can_be_triggered_again_after_release(self):
        ...
        self.assertEqual(vehicle_client.send_stop.call_count, 2)
...
class StopSignSharedLatchTests(unittest.TestCase):
    def test_stop_sign_still_auto_resumes_when_only_reason(self):
        ...
        vehicle_client.send_start.assert_called_once()
        self.assertFalse(controller.is_stopped)
    ...
    def test_fist_never_auto_resumes_even_as_sole_reason(self):
        ...
        vehicle_client.send_stop.assert_called_once()  # 이미 정지 상태라 재전송 안 됨
        self.assertTrue(controller.is_stopped)

        controller.release_stop("STOP_SIGN")  # 표지판만 사라짐 (STOP_SIGN은 release_stop 경로라 자동재출발 대상이나, FIST가 남아있어 억제됨)
        vehicle_client.send_start.assert_not_called()  # 주먹이 아직 남아있어 재출발 안 함
        self.assertTrue(controller.is_stopped)
...
class SpeedStepTests(unittest.TestCase):
    def test_accelerate_steps_and_rounds(self):
        controller, _, dashboard_client = make_controller()
    ...
    def test_decelerate_floors_at_min_speed_without_stopping(self):
        controller, vehicle_client, dashboard_client = make_controller()
        ...
        vehicle_client.send_stop.assert_not_called()  # 정지로 전환하지 않음 (버그 수정 사항)
        self.assertFalse(controller.is_stopped)
