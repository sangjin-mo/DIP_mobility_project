...
| PC 고정 IP | ~~지금은 미정으로 진행~~ → **2026-08-23 확정**: `central` 서버(=수신 서버+web_dashboard를 함께 띄우는 PC 한 대) IP를 `192.168.2.175`로 고정. `pi_agent/config.py`의 `SERVER_HOST`도 이 값으로 교체 완료. 개발자 노트북(이 리포지토리를 작업하는 PC)에서는 더 이상 `pc_server`/`web_dashboard`를 켜지 않고, central 서버 한 대만 켠다 |
...
- [x] **해결 (2026-08-23)**: `TRIGGER_ALLOWED_HOST` — PC 고정 IP(`192.168.2.175`)가 확정됐으므로 그 값으로 지정 가능해짐
- [ ] `system/testguide/테스트_가이드.md`가 아직 "PC(지금 이 컴퓨터)에서 `pc_server`를 직접 켜서 `localhost:8000`으로 테스트"하는 옛 구조를 전제로 쓰여 있음 — central 서버 분리 이후 구조에 맞게 갱신 필요
