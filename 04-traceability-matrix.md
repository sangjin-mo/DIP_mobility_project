...
| D_009 | Zones must be inferrable without DR's `ZONE_ENTER` events, since `drive_ver2` never sends them; a patrol's start/end must come from somewhere real | ADR-0009 | `pipeline/segment.py::segment_by_crop_type`/`dominant_crop_class`, `pipeline/aggregate.py::apply_crop_type_zone_names`, `web_dashboard/services/patrol_event_service.py` | `test_segment_by_crop_type_groups_images_by_dominant_class` and siblings (`tests/test_segment.py`); `test_apply_crop_type_zone_names_renames_zones_to_crop_display_names` and siblings (`tests/test_aggregate.py`); `test_start_patrol_posts_patrol_start_and_tracks_active_id` and siblings (`web_dashboard/tests/test_patrol_event_service.py`); `test_start_rover_posts_patrol_start_and_returns_patrol_id` (`web_dashboard/tests/test_app.py`) | A2 | ✓ — classification itself still needs `vision/image_analysis/system/classify.py` run against real captured images; nothing yet routes a specific patrol's webcam captures into it automatically (see ADR-0009's "Still open") |
