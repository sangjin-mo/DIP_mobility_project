...
| D_009 | Zones must be inferrable without DR's `ZONE_ENTER` events, since `drive_ver2` never sends them; a patrol's start/end must come from somewhere real | ADR-0009 | `pipeline/segment.py::segment_by_crop_type`/`dominant_crop_class`, `pipeline/aggregate.py::apply_crop_type_zone_names`, `web_dashboard/services/patrol_event_service.py` | `test_segment_by_crop_type_groups_images_by_dominant_class` and siblings (`tests/test_segment.py`); `test_apply_crop_type_zone_names_renames_zones_to_crop_display_names` and siblings (`tests/test_aggregate.py`); `test_start_patrol_posts_patrol_start_and_tracks_active_id` and siblings (`web_dashboard/tests/test_patrol_event_service.py`); `test_start_rover_posts_patrol_start_and_returns_patrol_id` (`web_dashboard/tests/test_app.py`); `test_end_patrol_triggers_classify_subprocess_for_todays_received_dir` and siblings (`web_dashboard/tests/test_patrol_event_service.py`); `test_rerun_skips_already_classified_images`, `test_same_second_frames_get_a_deterministic_order` (`tests/test_vision_classify.py`); `test_images_are_transferred_before_classification_is_spawned`, `test_classify_is_spawned_without_a_drive_window_filter` (`web_dashboard/tests/test_patrol_event_service.py`) | A2 | ✓ — `PatrolEventService.end_patrol` pulls pending images off the webcam Pi, waits for that transfer, then auto-triggers `vision/image_analysis/system/classify.py` against today's `received/{date}/`. Scoping is by classify.py's already-classified ledger, **not** by the drive window (ADR-0011: camera capture is independent of vehicle control, so the two windows do not overlap — the drive-window version classified 0 images on every real run). `DASHBOARD_AUTO_CLASSIFY_ENABLED` (default on) can disable it. Does not handle a patrol spanning midnight. |
| D_010 | Classification must be idempotent and self-releasing: a re-run must not re-bill or rebind already-classified images, and a run that dies at startup must still release `ai_report` | ADR-0011 | `vision/image_analysis/system/classify.py::image_id_for`/`_load_ledger`/`_write_analysis_atomically`, `ai_report/ingest/vis_watcher.py::scan_once`, `ai_report/cli.py::_make_patrol_end_trigger` | `test_image_ids_are_stable_regardless_of_completion_order`, `test_rerun_skips_already_classified_images`, `test_stale_complete_marker_is_cleared_before_reclassifying`, `test_complete_marker_is_written_even_when_the_run_fails_at_startup` (`tests/test_vision_classify.py`); `test_complete_is_read_before_the_glob`, `test_unreadable_analysis_file_is_skipped_not_fatal` (`tests/test_vis_watcher.py`) | A2 | ✓ |
...
|---|---|---|---|
| AI requirements | 13 | 13 | 0 — 12 ✓, AI_104 deferred (203 tests pass as of 2026-08-25) |
| Derived requirements | 10 | 10 | 0 — D_007 still WIP (empty fixtures) |
