...
| D_009 | Zones must be inferrable without DR's `ZONE_ENTER` events, since `drive_ver2` never sends them; a patrol's start/end must come from somewhere real | ADR-0009 | `pipeline/segment.py::segment_by_crop_type`/`dominant_crop_class`, `pipeline/aggregate.py::apply_crop_type_zone_names`, `web_dashboard/services/patrol_event_service.py` | `test_segment_by_crop_type_groups_images_by_dominant_class` and siblings (`tests/test_segment.py`); `test_apply_crop_type_zone_names_renames_zones_to_crop_display_names` and siblings (`tests/test_aggregate.py`); `test_start_patrol_posts_patrol_start_and_tracks_active_id` and siblings (`web_dashboard/tests/test_patrol_event_service.py`); `test_start_rover_posts_patrol_start_and_returns_patrol_id` (`web_dashboard/tests/test_app.py`); `test_end_patrol_triggers_classify_subprocess_for_todays_received_dir` and siblings (`web_dashboard/tests/test_patrol_event_service.py`); `test_after_before_ts_ms_restricts_to_images_in_that_window` (`tests/test_vision_classify.py`) | A2 | ✓ — `PatrolEventService.end_patrol` (2026-08-24) now auto-triggers `vision/image_analysis/system/classify.py` against that patrol's own `received/{date}/` images, scoped by `--after-ts-ms`/`--before-ts-ms` (its own recorded START/END epoch-ms) so images from other patrols the same day are excluded; `DASHBOARD_AUTO_CLASSIFY_ENABLED` (default on) can disable it. Does not handle a patrol spanning midnight. |
